...
def load_evolution_data_from_csv(csv_path):
    """
    Reads the pokemon_evolutions.csv and creates a dictionary mapping
    a pre-evolution's ID to its evolution level.
    """
    evolutions = {}
    with open(csv_path, 'r', encoding='utf-8') as f:
        reader = csv.DictReader(f)
        for row in reader:
            # We are only interested in evolutions triggered by leveling up (trigger_id '1')
            if row.get('evolution_trigger_id') == '1' and row.get('minimum_level'):
                try:
                    # The 'id' in this CSV refers to the pre-evolution
                    prevo_id = int(row['id'])
                    evo_level = int(row['minimum_level'])
                    evolutions[prevo_id] = evo_level
