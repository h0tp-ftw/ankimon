        ...
        try:
            ...
            if phase1_done:
                self._update_progress(
                    10, "Phase 1 (Collection) already completed. Skipping..."
                )
            else:
                ...
                if self.mypokemon_path.is_file() and not self.cancelled:
                    ...
                    for i, pokemon in enumerate(pokemon_list):
                        ...
                        if total > 0 and (i % 20 == 0 or i == total - 1):
                            ...
                            self._update_progress(
                                pct, f"Migrating Pokemon {i + 1}/{total}..."
                            )

                    if not self.cancelled:
                        self._update_progress(
                            50, f"✓ Migrated {stats['pokemon']} Pokemon"
                        )
                ...
                if self.badges_path.is_file():
                    ...
                    for badge in badges_list:
                        ...
                        if isinstance(badge, int):
                            badge_id = str(badge)
                            badge_data = {"id": badge, "achieved": True}
                        else:
                    ...
                    if not self.cancelled:
                        self._update_progress(
                            65, f"✓ Migrated {stats['badges']} badges"
                        )
            ...
            if self.history_path and self.history_path.is_file():
                ...
                for i, pokemon in enumerate(history_list):
                    ...
                    if total_hist > 0 and (i % 50 == 0 or i == total_hist - 1):
                        ...
                        self._update_progress(
                            pct, f"Migrating history {i + 1}/{total_hist}..."
                        )

                if not self.cancelled:
                    self._update_progress(
                        91, f"✓ Migrated {stats['history']} history entries"
                    )
            ...
            if self.data_path and self.data_path.is_file():
                ...
                if isinstance(user_data, dict):
                    ...
                    self.log_area.append(
                        f"  ⚠ Unexpected user data format: {type(user_data).__name__}"
                    )
