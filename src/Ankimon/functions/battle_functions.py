...
def _process_battle_effects(
    ...
    def get_pokemon_name(target_side: str) -> str:
        if target_side == "user":
            return (
                main_pokemon.display_name
                if (main_pokemon and hasattr(main_pokemon, "display_name"))
                else "Your Pokemon"
            )
        else:
            return (
                enemy_pokemon.display_name
                if (enemy_pokemon and hasattr(enemy_pokemon, "display_name"))
                else "Enemy Pokemon"
            )
    ...
    try:
        ...
        if battle_status in status_messages:
            return translator.translate(
                status_messages[battle_status], pokemon_name=main_pokemon.display_name
            )
        else:
