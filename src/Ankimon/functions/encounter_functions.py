    ...
    try:
        ...
        if team_data:
            for member_info in team_data:
                ind_id = member_info.get("individual_id")
                if ind_id and ind_id != main_pokemon.individual_id:
                    pk_dict = services.db.get_pokemon(ind_id)
                    if pk_dict:
                        from ..business import calculate_max_hp
                        pk_dict["hp"] = calculate_max_hp(pk_dict)
                        services.db.save_pokemon(pk_dict)
    except Exception as e:
        logger = services.logger
        if logger:
            logger.log("error", f"Error healing team on faint: {e}")
